...
public class RefundmentUtil {
     ...
     * @throws Exception
     */
    public void service() throws UnsupportedEncodingException, Exception {
        ...
        for (OrderOtherPaySchedule orderOtherPaySchedule : schedules) {
            ...
            if (orderOtherPays == null || orderOtherPays.size() == 0) {
    ...
    public void refundment(String orderCode, String orderPrice, String code) throws UnsupportedEncodingException, Exception {
        ...
        if (httpClient.call()) {
            ...
            if (resHandler.isTenpaySign() && "0".equals(retcode)) {
                /*
                 * 退款状态 refund_status 4，10：退款成功。 3，5，6：退款失败。 8，9，11:退款处理中。 1，2: 未确定，需要商户原退款单号重新发起。 7：转入代发，退款到银行发现用户的卡作废或者冻结了，导致原路退款银行卡失败，资金回流到商户的现金帐号，需要商户人工干预，通过线下或者财付通转账的方式进行退款。
                 */
                ...
                if ("4".equals(refund_status) || "10".equals(refund_status)) {
                    // 退款完成
                    OrderOtherPay otherPay = orderOtherPayService.selectOthertByOrderPayCode(out_refund_no);
