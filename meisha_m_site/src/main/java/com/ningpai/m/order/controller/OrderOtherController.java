...
public class OrderOtherController {
    ...
    public ModelAndView queryOther(OrderOtherPay otherPay, Long orderId, HttpServletRequest request) throws UnsupportedEncodingException {
        ...
        if (!"0".equals(order.getOrderStatus())) {
            return new ModelAndView(new RedirectView(OrderURL.ERRORPAGE)).addObject("message", "订单已支付");
        }
    ...
    public ModelAndView payOrderSingle(OrderOtherPay orderOtherPay, Long payId, HttpServletRequest request) throws UnsupportedEncodingException {
        ...
        try {
            ...
            orderOtherPay.setOrderPayName(new String(orderOtherPay.getOrderPayName().getBytes("ISO-8859-1"), ConstantUtil.UTF));
            orderOtherPay.setOrderPayRemark(new String(orderOtherPay.getOrderPayRemark().getBytes("ISO-8859-1"), ConstantUtil.UTF));
