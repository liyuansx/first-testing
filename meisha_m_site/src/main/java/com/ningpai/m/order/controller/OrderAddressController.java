...
public class OrderAddressController {
    ...
    public ModelAndView queryOrderAddressList(HttpServletRequest request, Long[] box, Long addressId, Long deliveryPointId, Long ch_pay) {
//        if (ch_pay != null) {
//            request.getSession().setAttribute("ch_pay", ch_pay);
//        }
//        List<ShoppingCart> shoplist = shoppingCartService.searchByProduct(request, box);
//        //存放第三方店铺标示
...
//                if ("0".equals(sc.getGoodsDetailBean().getProductVo().getThirdId().toString())) {
//                    thirdIds.add(0L);
//                }
//            }
//        }
    ...
    public ModelAndView orderFillAddress(HttpServletRequest request, Long addressId, Long[] box) {
        if (LoginUtil.checkLoginStatus(request)) {
            if (addressId == null) {
                return seoService.getCurrSeo(new ModelAndView(OrderURL.FILLADDRESS).addObject("box", box)).addObject("addressId", addressId);
            } else {
                return seoService.getCurrSeo(new ModelAndView(OrderURL.FILLADDRESS).addObject("addr", addressService.queryCustomerAddressById(addressId, (Long) request.getSession().getAttribute(CustomerConstants.CUSTOMERID))).addObject("box", box));
            }
    ...
    public ModelAndView orderUpdateAddress(HttpServletRequest request, CustomerAddress address, Long[] box) {
        if (LoginUtil.checkLoginStatus(request)) {
//            try {
//                address.setAddressAlias(new String(address.getAddressAlias().getBytes("ISO-8859-1"), ConstantUtil.UTF));
//                address.setAddressName(new String(address.getAddressName().getBytes("ISO-8859-1"), ConstantUtil.UTF));
//                address.setAddressDetail(new String(address.getAddressDetail().getBytes("ISO-8859-1"), ConstantUtil.UTF));
...
//            }
            ...
            return new ModelAndView("redirect:/order/addresslist.html").addObject("box", box).addObject("addressId", address.getAddressId());
        } else {
            return seoService.getCurrSeo(new ModelAndView(CustomerConstants.REDIRECTLOGINTOINDEX));
        }
    }
    ...
    public ModelAndView delAddress(HttpServletRequest request,Long addressId){
        ...
        if(customerId!=null){
            ...
            //修改地址状态 为删除
            addressService.updateAddress(address,customerId);
            return new ModelAndView("redirect:/order/addresslist.html");
        }else{
            //去登录
            return seoService.getCurrSeo(new ModelAndView(CustomerConstants.REDIRECTLOGINTOINDEX));
        }
...
//            }
            ...
            return new ModelAndView("redirect:/order/addresslist.html").addObject("box", box).addObject("addressId", addressId);
        } else {
            return seoService.getCurrSeo(new ModelAndView(CustomerConstants.REDIRECTLOGINTOINDEX));
        }
    }
