...
public class GenerateLinkUtils {
    ...
    public static String generateCheckEmailLink(HttpServletRequest request, Customer user) {
        // 获得客户端发送请求的完整url
        String url = request.getRequestURL().toString();
        return url.substring(0, url.lastIndexOf("/")) + "/valididemail.html?" + CHECK_CODE + "=" + generatePwdCheckcode(user) + "&type=" + request.getSession().getAttribute("utype") + "&d=" + user.getCustomerId();
    }
