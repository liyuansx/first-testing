...
public class WXActivityController {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (auth != null) {
            String url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" + auth.getAuthClientId() + "&redirect_uri=" + auth.getAuthRedirectUri() + "&response_type=code&scope=snsapi_base&state=1#wechat_redirect";
            try {
    ...
    protected ModelAndView getWXToken(HttpServletRequest request, HttpServletResponse response) throws HttpException, IOException {
        // 获取code
        String code = request.getParameter("code");
        if (code == null) {
            return new ModelAndView("redirect:/activity.html");
        }
        // 获取微信接口AppId...
        Auth auth = authService.findAuthByAuthType("8");
        if (auth != null) {
