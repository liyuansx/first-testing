...
public class WXSendMSG {
    ...
    public static boolean sendWxMsgForOrderCreate(Map<?, ?> paraMap, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            postMethod.setRequestBody("{ \"touser\":  \"" + paraMap.get(ConstantUtil.OPENID) + "\" ," + "\"url\" :\"http:\\/\\/shop.ningpai.com\\/mobile\\/getwxcode3.htm?toUrl=customer/detail-" + paraMap.get(ConstantUtil.ORDERID) + ".html\"," + "\"topcolor\" :\"#28e650\", " + "\"template_id\" :\"A6xE6PJ4B60PY25lYfLqrDJClmeP8oBMSqwjQ0yQO_w\",\"data\":{" + "\"first\" :{\"value\":\"您的订单已提交成功,以下是订单信息:\"}, " + "\"orderID\" :{\"value\":\"" + paraMap.get("orderNo") + "\"} ," + "\"orderMoneySum\" :{\"value\":\"" + paraMap.get("orderPrice") + "元\"}, " + "\"backupFieldName\" :{\"value\":\"商品名称\"}, " + "\"backupFieldData\" :{\"value\":\"" + paraMap.get("goodsName") + "\"}, " + "\"remark\" :{\"value\":\"如有问题请致电400-880-1024或直接在微信留言，福气商城将第一时间为您服务！\"}" + "}}");
    ...
    private static void throwNullPoniter(HttpServletRequest request,
    ...
    public static boolean sendWxMsgForOrderPaySucc(Map<?, ?> paraMap, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            postMethod.setRequestBody("{ \"touser\":  \"" + paraMap.get(ConstantUtil.OPENID) + "\" ," + "\"url\" :\"http:\\/\\/shop.ningpai.com\\/mobile\\/getwxcode3.htm?toUrl=customer/detail-" + paraMap.get(ConstantUtil.ORDERID) + ".html\"," + "\"topcolor\" :\"#28e650\", " + "\"template_id\" :\"dB37CTNFdyXwZTz0mspVvWxLzY4agsBSW3Grujkn5wg\",\"data\":{" + "\"first\" :{\"value\":\"我们已收到您订单编号为" + paraMap.get("orderNo") + "的货款，开始为您打包商品，请耐心等待: )\"}, " + "\"orderMoneySum\" :{\"value\":\"" + paraMap.get("orderPrice") + "元\"}, " + "\"orderProductName\" :{\"value\":\"" + paraMap.get("goodsName") + "\"}, " + "\"Remark\" :{\"value\":\"如有问题请致电400-880-1024或直接在微信留言，福气商城将第一时间为您服务！\"}" + "}}");
            HttpClient mClite = new HttpClient();
            postMethod.getParams().setContentCharset(ConstantUtil.UTF);
            mClite.executeMethod(postMethod);
            resMap = WeiXinUtil.getWxSendResponse(postMethod.getResponseBodyAsString());
    ...
    public static int sendWxMsgForOrderCreate(HttpServletRequest request, HttpServletResponse response, String orderNo) {
        ...
        if (accessToken != null && orderNo != null) {
            String url = "https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token=" + accessToken + "&touser=" + openId + "&msgtype=text&content=您好,您的订单已经成功创建,订单编号:" + orderNo;
    ...
    public static int sendWxMsgForOrderPaySucc(HttpServletRequest request, HttpServletResponse response, String orderNo, Double orderPrice) {
        ...
        if (accessToken != null && orderNo != null) {
            String url = "https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token=" + accessToken + "&touser=" + openId + "&msgtype=text&content=您好,您的订单" + orderNo + "支付成功,订单金额:" + orderPrice;
            PostMethod postMethod = new PostMethod(url);
            HttpClient clientToken = new HttpClient();
            postMethod.getParams().setContentCharset(ConstantUtil.UTF);
            String res = "";
