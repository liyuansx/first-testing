...
public class CustomerAddressControllerM {
    ...
    public ModelAndView queryCustomerAddress(HttpServletRequest request) {
        if (LoginUtil.checkLoginStatus(request)) {
            return seoService.getCurrSeo(new ModelAndView(CustomerConstants.CUSTOMERADDRESS).addObject("addresses", customerAddressService.queryCustomerAddress((Long) request.getSession().getAttribute(CustomerConstants.CUSTOMERID))));
        } else {
    ...
    public ModelAndView fillAddress(HttpServletRequest request, Long addressId) {
        if (LoginUtil.checkLoginStatus(request)) {
            if (addressId == null) {
                ...
                return seoService.getCurrSeo(new ModelAndView(CustomerConstants.FILLADDRESS).addObject("addr", customerAddressService.queryCustomerAddressById(addressId, (Long) request.getSession().getAttribute(CustomerConstants.CUSTOMERID))));
            }
