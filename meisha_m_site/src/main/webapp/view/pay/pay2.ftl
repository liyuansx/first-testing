...
<html lang="zh-cn">
  <head>
  	<#assign basePath=request.contextPath>
        ...
        <script src="${basePath}/plugin/wxpay/md5.js"></script>
        <script src="${basePath}/plugin/wxpay/sha1.js"></script>
    ...
    <script Language="javascript">
            ...
            {
                return "9b65423e573b0baa90e882e7158270ce";
            }
            ...
            function getPackage()
            {
                ...
                var notify_url = document.form1.notifyUrl.value;//支付成功后将通知该地址
                var out_trade_no = document.form1.tradeno.value;//订单号，商户需要保证该字段对于本商户的唯一性
                var partner = getPartnerId();//测试商户号
                var spbill_create_ip = document.form1.custIp.value;//用户浏览器的ip，这个需要在前端获取。这里使用127.0.0.1测试值
            ...
            function getAppKey()
            {
                return "LTD5WYXxE8xbFAghepFKezy04NaqXDojDEeFFV5ZgFfhVXX8zNF98yS4kvsTGhlTx2wGRU3JJsEIdnawrAX0AG06dRQ5VaFMyfndnO6ZcremCphgFlJhKwg0dvCWru2e";
            }
        			...
        			WeixinJSBridge.invoke('getBrandWCPayRequest',{
                                ...
                                "paySign" : getSign() //微信签名
                                      ...
                                      if(res.err_msg == "get_brand_wcpay_request:ok" ) {
                                      	...
                                      	location.href="${basePath}/order/otherpaysinglesuccess.html?orderId="+orderId;
                                      }else{
	                                    var orderId=$("#orderId").val();
                                      	location.href="${basePath}/otherpaysingle-"+orderId+".html";
                                      }
        ...
        <form name="form1" target="_blank" style="display:none" >
            <table border="1">
                <TR><th>公众号ID</th> <th><INPUT value="wx54348d130baacafd" name="appId" id="1"></th>
                <tr><th>商户ID</th><th><INPUT value="1220343401" name="partnerId" id="2"></th>
                <TR><th>总金额</th><th><INPUT value=${payPrice} name="totalFee" id="3"></th>
                <TR><th>商品名</th><th><INPUT value="${goodsInfoName}" name="body" id="4"></th>
                <TR><th>订单编号</th><th><INPUT value="${order.orderCode}" name="tradeno" id="5"></th>
                <TR><th>客户IP</th><th><INPUT value="${ip}" name="custIp" id="6"></th>
                <TR><th>通知地址</th><th><INPUT value="${callBackUrl}" name="notifyUrl" id="7"></th>
                <TR><th>订单id</th><th><INPUT value="${orderId}" name="orderId" id="orderId"></th>
