...
<html lang="zh-cn">
	        	...
	        	$(function(){
		        	...
		        	$.get("getCount.htm",function(data){
		        		...
		        		if(data>=1){
				        	...
				        	$(".curtain_wp").width($(window).width()).height($(window).height()).delay(1000).fadeOut(200,function(){
					        	showmain();
				        	});
		        		}
		        	});
  ...
  <style>
      ...
      .ps-head{background: url(http://st.360buyimg.com/m/images/index/header-bg.png?v=2) repeat-x #efefef;  background-size: 100% 44px;}
		...
		<div id="dingbu" class="dingbufix">
			<div class="m_nav">
				...
				<a href="${basePath}/main.html">
					<div class="recommend "><span class="recommend-selected">推荐</span></div>
				</a>
				<#--
				...
				<a href="${basePath}/myshoppingmcart.html">
					<div class="m_icon iconfont cart_bolder mfr">&#xe622;</div>
				</a>
			</div>
			<div class="search">
			 <form action="${basePath}/searchProduct.htm" method="post" id="searchProductForm" style="margin-bottom:0;">
              <input type="text" value="<#if keyWords??>${keyWords}</#if>" class="m_search" placeholder="告诉麦子想买点什么.." id="title" name="keyWords"/>
