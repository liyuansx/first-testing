...
<html lang="zh-cn">
  ...
  <body>
    ...
    <div class="infor_detail" style="margin-top:1em;">
        <div class="fill_infor">
             <span>付款金额</span>
             <input type="text" name="orderPayPrice" class="orderPayPrice" placeholder="最多可代付${(map.orderSchedule.orderResiduePrice)?string("0.00")}元"/>
             <button type="button" class="btn btn-default fr xiugai">修改</button>
        </div><!--fill_infor-->
    </div>
    <div class="pay_area">
    ...
    <div class="modal fade" role="dialog" id="goods_tips">
     ...
     <input type="hidden" value="${map.orderSchedule.orderResiduePrice}" class="order_r_p">
    ...
    <script>
      ...
      function subForm(){
		  if($(".orderPayPrice").val()!=""){
		  	if($(".order_r_p").val()-$(".orderPayPrice").val()>=0){
			  	$(".subForm").submit();
		  	}else{
		  		$(".tip_text").html("金额超出");
		  		$(".tan_k").click();
