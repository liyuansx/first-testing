...
<html lang="zh-cn">
 ...
 <script type="text/javascript" src="${basePath}/js/shoppingcart/jsOperation.js"></script>
  ...
  <form action="order/payorder.html"  class="payOrder"  method="post">
     ...
     <h4 class="text-center"><img height="20" alt="" src="${basePath}/images/success_icon2.png">&nbsp;订单提交成功！<small>请尽快完成付款</small></h4>
...
<#list orders as order>
    ...
    <div class="settle">
	      <p><strong>实付：￥${(sumprice?number)?string("0.00")}</strong></p>
    </div><!-- /settle -->
<#if pid??&&pid=='1'  ||thirdpid??&&thirdpid=='1'>
    <div class="wrap">
       ...
       <button type="button" id="wxPayBtn" onclick="getWXParam(${order.orderId});" class="btn btn-success btn-lg btn-block" >微信支付</button>
        ...
        <#if paylist??>
            <#list paylist as pay>
                <#if (pay.payType='1')>
                    <button type="button" id="zfbPayBtn" paydata="${pay.payId}"  onclick="changethis(${pay.payId});" class="btn btn-danger btn-lg btn-block">支付宝支付</button>
                </#if>
             </#list>
        </#if>
    </div>
    ...
    <script>
     ...
     function otherPay(obj){
	  ...
	  $(function(){
	  	//kidd修改注释   跳转到支付宝页面
		var paydata=$("#zfbPayBtn").attr("paydata");
		changethis(paydata);
